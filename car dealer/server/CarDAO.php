...
$id = $carDealer_data[0]->getCarDealerId();
        $db = Dbh::getInstance();
        $stmt = $db->prepare($sql." Where cardealerId=".$id);
        $stmt->execute();
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $car[] = new Vehicle(
                ...
                $row["status"],
                $row["dateAdded"],
                $carDealer_data[0]->getCarDealerId()
            );
        }
        $stmt = null;
        return $car;
...
$id = $carDealer_data[0]->getCarDealerId();
    ...
    function isCreated($postArray) {
        ...
        $car = new Vehicle(
            ...
            $this->stringValue($carDealer_data[0]->getCarDealerId())
        );
        if($this->create($car)) {
            return 1;
        } else {
